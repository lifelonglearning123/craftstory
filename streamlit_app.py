...
def describeImage(image_bin, language):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an expert art critic with extensive knowledge of art history, techniques, styles, and movements. Analyze artworks with an emphasis on composition, color theory, technique, historical context, and artistic merit."},
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"Analyze this artwork in detail in {language}. Include: style, technique, composition, color palette, emotional impact, and historical context if relevant."},
...
#generate better craft story
def generate_story(image_description, relevant_context, language):
    """Generate marketing content for an artwork based on its description."""
    prompt = f"""
    You are an art marketing specialist with expertise in creating compelling content that highlights the value, uniqueness, and investment potential of artworks.
    
    Based on this artwork description, create marketing materials:
    {image_description}
    
    Please include:
    1. A catchy headline
    2. A compelling 2-3 sentence description for a gallery card
    3. Three key selling points that highlight what makes this artwork special
    4. Target collector demographic
    5. Suggested price range positioning (premium, mid-range, accessible)
    6. Social media copy (Instagram, Twitter, Facebook)
    7. Keywords for digital marketing
...
with st.sidebar:
    try:
        ...
        try:
            ...
            st.image('./customizations/logo/default.svg', use_container_width=True)
            st.text('')
...
with st.sidebar:
    ...
    # Add a subheader for the image analysis section
    st.subheader(lang_dict.get('artwork_analysis', 'Artwork Analysis'))
    
    # Option 1: Take a picture with camera
    picture = st.camera_input(lang_dict['take_picture'])
    
    # Option 2: Upload an image file
    uploaded_image = st.file_uploader(lang_dict.get('upload_artwork', 'Upload Artwork'), 
                                     type=['jpg', 'jpeg', 'png', 'webp'])
    
    # Process either the camera picture or uploaded image
    ...
    if image_to_process:
        ...
        relevant_documents = retriever.get_relevant_documents(picture_desc)
        context = "\n".join([doc.page_content for doc in relevant_documents])

        # Step 3: Generate a story
        story = generate_story(picture_desc, context, language)

        # Step 4: Display the story
        st.subheader("Generated Story")
        st.write(story)
        #picture_desc = response.choices[0x].message.content
        question = story
